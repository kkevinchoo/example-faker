...
try {
    require_once 'vendor/autoload.php'; // Carga la biblioteca Faker
    ...
    // Generar datos de clientes y llenar la tabla
    for ($i = 0; $i < 500; $i++) {
    ...
    echo "Datos de clientes generados y agregados correctamente\n";
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Tabla de productos creada con éxito";
    } else {
    ...
    foreach ($data['products'] as $product) {
        ...
        $sql = "INSERT INTO productos (title, description, price, discountPercentage, rating, stock, brand, category, thumbnail, images) 
            VALUES ('$title', '$description', '$price', '$discountPercentage', '$rating', '$stock', '$brand', '$category', '$thumbnail', '$images');";

        if ($conn->query($sql) !== TRUE) {
            echo "Error al insertar datos en la tabla de productos: " . $conn->error;
        }
    }
    echo "Datos de productos generados y agregados correctamente\n";
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Tabla ventas creada con éxito\n";
    } else {
        echo "Error creando tabla: " . $conn->error;
    }
    // Generar datos de ventas y llenar la tabla
    for ($i = 0; $i < 1000; $i++) {
        $cliente_id = $faker->numberBetween(1, 500);
        $producto_id = $faker->numberBetween(1, 30);
        $red_social = $faker->randomElement(['Facebook', 'Twitter', 'Instagram', 'LinkedIn']);
        $sql = "INSERT INTO ventas (cliente_id, producto_id, red_social)
        VALUES ('$cliente_id', '$producto_id', '$red_social')";
        echo $sql;
        $conn->query($sql);
    }
    echo "Datos de ventas generados y agregados correctamente\n";
    // Crear columna para registro de visualización de página en la tabla de clientes
    $sql = "ALTER TABLE clientes
        ADD COLUMN se_hizo_cliente BOOLEAN";
    if ($conn->query($sql) === TRUE) {
        echo "Columna se_hizo_cliente agregada con éxito\n";
    } else {
        echo "Error agregando columna: " . $conn->error;
    }
    // Actualizar la tabla de clientes con datos aleatorios para la columna se_hizo_cliente
    $sql = "UPDATE clientes
        SET se_hizo_cliente = " . $faker->boolean(50);
    $conn->query($sql);
    echo "Datos de visualización de página de clientes actualizados correctamente\n";
    $conn->close();
} catch (\Throwable $th) {
    var_dump($th);
}
